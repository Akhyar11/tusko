...
12. `12-agent-testing-proof-and-verification.sh`: OpenCode AI Auditor yang memverifikasi bahwa Agent AI telah melakukan pengetesan nyata terhadap setiap perubahan kode (Backend dengan Automated Suite Test `php artisan test` & Frontend dengan Browser Test riil menggunakan **Playwright dari Python** (`python3` + paket `playwright`, browser Chromium)). **Kewajiban Pengujian Fitur Aktif**: Jika perubahan kode menyangkut suatu fitur (form, filter drawer, aksi tabel, modal status, mutasi stok, alur belanja/checkout, dll.), pengujian frontend DILARANG hanya bersifat pasif memuat halaman (smoke test), melainkan WAJIB sampai menguji penggunaan fitur tersebut secara langsung (pengetikan form kustom, pemilihan dropdown, klik tombol aksi, eksekusi filter, submit, dan verifikasi perubahan state/DOM). Bukti wajib didokumentasikan di folder `.agent-test-proofs/` sebelum commit dan diverifikasi keabsahannya oleh AI. Berkas bukti dipertahankan selama pre-commit dan **dibersihkan secara otomatis oleh hook Git post-commit** setelah proses commit berhasil dilakukan. Cakupan triple: staged + unstaged + untracked. **TOOLING WAJIB**: Seluruh pengujian browser WAJIB dijalankan dengan **Playwright dari Python** (`python3` + paket `playwright`, browser Chromium). DILARANG memakai curl/HTTP-only, tool manual, framework non-Playwright, atau sekadar memuat URL sebagai bukti pengujian fitur frontend. Jika Agent ditolak (REJECTED) oleh auditor ini, Agent WAJIB mengulang pengujian menggunakan Playwright dari Python.
...
# Protokol Korelasi Tugas dengan `taks.txt`

1. **Rujukan Pertama Wajib**: Pada SETIAP permintaan tugas/pekerjaan dari user (mis. "kerjakan", "buat", "perbaiki", "tambah", "ubah", "lanjut"), langkah PERTAMA Agent WAJIB merujuk ke file `taks.txt` di root proyek (panduan kanonis pengerjaan) SEBELUM melakukan tindakan apa pun.
2. **Cek Korelasi**: Agent WAJIB memeriksa korelasi antara perintah user dengan MAJOR TASK (T00–T25) dan MINOR TASK (`Txx.y`) di `taks.txt` (termasuk file/endpoint/tabel yang disebut user).
3. **Konfirmasi Wajib**: Jika terdapat korelasi, Agent WAJIB KONFIRMASI terlebih dahulu kepada user mengenai task mana yang akan dikerjakan (sebutkan Task ID, judul, dan bagian fitur/DB/FE/API/test). DILARANG mulai menulis kode sebelum konfirmasi user.
4. **Tanpa Korelasi**: Jika tidak ada task yang cocok, Agent WAJIB memberi tahu dan menawarkan untuk menambah major/minor task baru ke `taks.txt` (dengan struktur Fitur | DB Schema | Perilaku FE | API | Test) ATAU mengerjakan di luar `taks.txt` — dengan konfirmasi user lebih dulu.
