...
while IFS= read -r f; do
    ...
    MATCHES=$(grep -m 120 -n -E 'className|<select|<input|<textarea|<button|<h1|<h2|<label|ServerSideSelect|FormTipsPanel|lg:col-span|limitOptions|selectable|MoreVertical|bulkActions' "$f")
    if [ -n "$MATCHES" ]; then
