...
export default function PopularChipsBar({ onSelectChip = () => {} }) {
  ...
  return (
    <section className="bg-neutral-100 border-b border-neutral-200 py-3 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto flex items-center gap-2.5 overflow-x-auto no-scrollbar">
        ...
        {popularKeywords.map((item, idx) => (
          <button
            ...
            className="whitespace-nowrap bg-white hover:bg-black hover:text-white border border-neutral-300 text-[11px] font-bold uppercase px-3 py-1.5 transition-colors shadow-2xs flex-shrink-0 cursor-pointer"
          >
            {item.label}
          </button>
        ))}
