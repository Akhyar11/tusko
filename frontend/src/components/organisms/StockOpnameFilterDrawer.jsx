...
/**
 * Organism: StockOpnameFilterDrawer
 * Sidebar filter kanan-ke-kiri sesi Stock Opname (T25.2) — 100% kolom tabel.
 */
export default function StockOpnameFilterDrawer({
  isOpen = false,
  onClose = () => {},
  activeFilterCount = 0,
  searchQuery = '',
  ...
  const [warehouseOptions, setWarehouseOptions] = useState([{ value: 'all', label: 'Semua Gudang' }]);

  useEffect(() => {
    let active = true;
    warehouseService.fetchWarehouses({ all: 1 })
      .then((res) => {
        if (!active) return;
        setWarehouseOptions([
          { value: 'all', label: 'Semua Gudang' },
          ...(res.data || []).map((w) => ({ value: String(w.id), label: `${w.name} (${w.code})` }))
        ]);
      })
      .catch(() => {});
    return () => { active = false; };
  ...
  const statusOptions = [
    ...
    { value: 'in_progress', label: 'Diajukan (In Progress)' },
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-base font-black font-sport uppercase tracking-wider text-white">Filter Stok Opname</h2>
                  {activeFilterCount > 0 && (
                ...
                <p className="text-xs text-neutral-400 mt-0.5">Saring sesi opname berdasarkan nomor, gudang, status, tanggal & jumlah item</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
            <div className="space-y-2">
              <label className="block text-xs font-sport font-black uppercase tracking-wider text-neutral-900">Nomor Opname</label>
              <SearchBar value={searchQuery} onChange={onSearchQueryChange} placeholder="Contoh: SO/25092026/001..." />
            </div>

            <div className="space-y-2">
              <label className="block text-xs font-sport font-black uppercase tracking-wider text-neutral-900">Gudang</label>
              <ServerSideSelect options={warehouseOptions} value={warehouseFilter} onChange={onWarehouseFilterChange} placeholder="Pilih gudang opname..." />
            </div>

            <div className="space-y-2">
              <label className="block text-xs font-sport font-black uppercase tracking-wider text-neutral-900">Status</label>
              <ServerSideSelect options={statusOptions} value={statusFilter} onChange={onStatusFilterChange} placeholder="Pilih status opname..." />
            </div>

            <div className="space-y-2">
              <label className="block text-xs font-sport font-black uppercase tracking-wider text-neutral-900">Tanggal Pelaksanaan</label>
              <div className="grid grid-cols-2 gap-3">
                <TextInput type="date" value={conductedFrom} onChange={onConductedFromChange} weight="mono" title="Dari tanggal" />
                <TextInput type="date" value={conductedTo} onChange={onConductedToChange} weight="mono" title="Sampai tanggal" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <ServerSideSelect options={sortOptions} value={sortFilter} onChange={onSortFilterChange} placeholder="Pilih urutan..." />
            </div>
          </div>
